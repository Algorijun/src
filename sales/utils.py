...
import numpy as np
import seaborn as sns # It works but why u are insisting that u can't find the seaborn?
...
def get_chart(chart_type, data, results_by,  **kwargs):
    ...
    plt.switch_backend('AGG') # ?? It's extremely //
    ...
    if chart_type == '#1':
        ...
        print('line chart')
        plt.plot(d[key], d[key].values)
    else:
        print(' whats going on here?')
    plt.tight_layout() # adjust size 
    chart = get_graph()
    return chart